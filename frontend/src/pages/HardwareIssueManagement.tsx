...
export default function HardwareIssueManagement() {
  ...
  useEffect(() => {
    ...
    async function fetchItems() {
      try {
        const res = await api.get('/issues/hardware');
        setItems(res.data);
      } catch (err) {
  ...
  const fetch = () => api.get('/issues/hardware').then(r=>setItems(r.data));

  const create = () => {
    api.post('/issues/hardware',{ descripcion:newDesc }).then(()=>{ setNewDesc(''); fetch(); });
  };
  const update = () => {
    if(editing==null) return;
    api.put(`/issues/hardware/${editing}`,{ descripcion:editDesc }).then(()=>{
      setEditing(null); fetch();
    });
  };
  const remove = (id:number) => {
    if(window.confirm('Eliminar?')) api.delete(`/issues/hardware/${id}`).then(fetch);
  };

  return (
    <div className="max-w-3xl mx-auto bg-gray-50 p-6 rounded-2xl shadow-lg space-y-6">
  {/* Título */}
  <h1 className="text-3xl font-bold text-gray-800">Gestor Hardware Issues</h1>
