...
export default function OtherIssueManagement() {
  ...
  useEffect(() => {
    ...
    async function fetchItems() {
      try {
        const res = await api.get('/issues/other');
        setItems(res.data);
      } catch (err) {
  ...
  const fetch = () => api.get('/issues/other').then(r=>setItems(r.data));

  const create = () => {
    api.post('/issues/other',{ descripcion:newDesc }).then(()=>{ setNewDesc(''); fetch(); });
  };
  const update = () => {
    if(editing==null) return;
    api.put(`/issues/other/${editing}`,{ descripcion:editDesc }).then(()=>{
      setEditing(null); fetch();
    });
  };
  const remove = (id:number) => {
    if(window.confirm('Eliminar?')) api.delete(`/issues/other/${id}`).then(fetch);
  };

  return (
    <div className="max-w-3xl mx-auto bg-gray-50 p-6 rounded-2xl shadow-lg space-y-6">
  {/* Título */}
  <h1 className="text-3xl font-bold text-gray-800">Gestor Other Issues</h1>
